/*
 * TBSP minimal receiver: blocking I/O, in-order DATA frames, sends ACK.
 * Usage: sudo ./tbsp_receiver <interface>
 * Example: sudo ./tbsp_receiver en5
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "TBSP minimal receiver on %s. Waiting for DATA frames.\n", ifname);

    for (;;) {
        ssize_t n = read(bpf_fd, read_buf, sizeof(read_buf));
        if (n <= 0) {
            perror("read");
            break;
        }
        ...
        while (ptr + sizeof(struct bpf_hdr) <= (char *)read_buf + n) {
            ...
            if (th->type == TBSP_TYPE_DATA) {
                ...
                if (seq != expected_seq) {
                    /* Out-of-order: drop (minimal: no NACK) */
                    goto next;
                }
                ...
                /* Reply with ACK; receiver uses sender's src as our dst */
                uint8_t dst[6], src[6];
                memcpy(dst, pkt + 6, 6);
                memcpy(src, pkt, 6);
                send_ack(bpf_fd, dst, src);
                free_slots = WINDOW_SIZE; /* Refill after ACK */
            }
